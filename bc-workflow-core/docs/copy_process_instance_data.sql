...
$BODY$
	...
	BEGIN
		...
		IF not found THEN
			...
			_r := _r || '"deployment_id": "' || _deployment_id || '", "proc_def_id": "' || _proc_def_id || '", "proc_inst_id": "' || $1 || '"';
		END IF;
		...
		insert into act_re_deployment(id_, name_, deploy_time_)
			select * from dblink(_dblink_name, 'select * from act_re_deployment where id_ = ''' || _deployment_id || '''')
			as t(id_ character varying(64), name_ character varying(255), deploy_time_ timestamp)
			where not exists (select 0 from act_re_deployment where id_ = _deployment_id);
		...
		insert into act_ge_bytearray(id_, rev_, name_, deployment_id_, bytes_, generated_)
			select * from dblink(_dblink_name, 'select * from act_ge_bytearray where deployment_id_ = ''' || _deployment_id || '''')
			as t(id_ character varying(64),
				...
				bytes_ bytea,
				generated_ boolean)
			where not exists (select 0 from act_ge_bytearray r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO act_re_procdef(id_, rev_, category_, name_, key_, version_, deployment_id_, 
			...
			select * from dblink(_dblink_name, 'select * from act_re_procdef where id_ = ''' || _proc_def_id || '''')
			as t(id_ character varying(64), 
					rev_ integer,
			...
			where not exists (select 0 from act_re_procdef where id_ = _proc_def_id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO act_ru_execution(id_, rev_, proc_inst_id_, business_key_, parent_id_, proc_def_id_, 
			...
			select * from dblink(_dblink_name, 'select * from act_ru_execution where proc_inst_id_ = ''' || $1 || '''')
			as t(id_ character varying(64),
				rev_ integer,
				...
				suspension_state_ integer)
			where not exists (select 0 from act_ru_execution r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO act_ru_task(id_, rev_, execution_id_, proc_inst_id_, proc_def_id_, name_, 
			...
			select * from dblink(_dblink_name, 'select * from act_ru_task where proc_inst_id_ = ''' || $1 || '''')
			as t(id_ character varying(64),
				rev_ integer,
			...
			where not exists (select 0 from act_ru_task r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO act_ru_identitylink(id_, rev_, group_id_, type_, user_id_, task_id_)
			select * from dblink(_dblink_name, 'select l.* from act_ru_identitylink l 
			...
			where not exists (select 0 from act_ru_identitylink r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		insert into act_ge_bytearray(id_, rev_, name_, deployment_id_, bytes_, generated_)
			select * from dblink(_dblink_name, 'select * from act_ge_bytearray where id_ in (
		...
		INSERT INTO act_ru_variable(id_, rev_, type_, name_, execution_id_, proc_inst_id_, task_id_, 
			...
			select * from dblink(_dblink_name, 'select * from act_ru_variable where proc_inst_id_ = ''' || $1 || '''')
			as t(id_ character varying(64),
				rev_ integer,
				type_ character varying(255),
				name_ character varying(255),
			...
			where not exists (select 0 from act_ru_variable r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO act_hi_actinst(id_, proc_def_id_, proc_inst_id_, execution_id_, act_id_, act_name_, 
			...
			select * from dblink(_dblink_name, 'select * from act_hi_actinst where proc_inst_id_ = ''' || $1 || '''')
			as t(id_ character varying(64),
				proc_def_id_ character varying(64),
				proc_inst_id_ character varying(64),
				execution_id_ character varying(64),
				...
				duration_ bigint)
			where not exists (select 0 from act_hi_actinst r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO act_hi_procinst(id_, proc_inst_id_, business_key_, proc_def_id_, start_time_, 
			...
			select * from dblink(_dblink_name, 'select * from act_hi_procinst where id_ = ''' || $1 || '''')
			as t(id_ character varying(64),
				proc_inst_id_ character varying(64),
				business_key_ character varying(255),
				proc_def_id_ character varying(64),
			...
			where not exists (select 0 from act_hi_procinst r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO act_hi_taskinst(id_, proc_def_id_, task_def_key_, proc_inst_id_, execution_id_, 
			...
			select * from dblink(_dblink_name, 'select * from act_hi_taskinst where proc_inst_id_ = ''' || $1 || '''')
			as t(id_ character varying(64),
				proc_def_id_ character varying(64),
				task_def_key_ character varying(255),
				proc_inst_id_ character varying(64),
				...
				priority_ integer,
				due_date_ timestamp without time zone)
			where not exists (select 0 from act_hi_taskinst r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO act_hi_detail(id_, type_, proc_inst_id_, execution_id_, task_id_, act_inst_id_, 
			...
			select * from dblink(_dblink_name, 'select * from act_hi_detail where proc_inst_id_ = ''' || $1 || '''')
			as t(id_ character varying(64),
				type_ character varying(255),
				proc_inst_id_ character varying(64),
				execution_id_ character varying(64),
				...
				text_ text,
				text2_ character varying(4000))
			where not exists (select 0 from act_hi_detail r where r.id_ = t.id_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_identity_actor_detail(id, create_date, work_date, iso, sex, card, duty_id, comment_)
			select * from dblink(_dblink_name, 'select * from bc_identity_actor_detail')
			as t(id integer,
			...
			where not exists (select 0 from bc_identity_actor_detail r where r.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_identity_actor(id, uid_, type_, status_, inner_, code, name, py, order_, email, 
			...
			select * from dblink(_dblink_name, 'select * from bc_identity_actor')
			as t(id integer,
			...
			where not exists (select 0 from bc_identity_actor r where r.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_identity_actor_history(id, pid, current, rank, create_date, start_date, end_date, actor_type, 
			...
			select * from dblink(_dblink_name, 'select * from bc_identity_actor_history')
			as t(id integer,
				pid integer,
				current boolean,
				rank integer,
			...
			where not exists (select 0 from bc_identity_actor_history r where r.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_identity_auth(id, password)
			select * from dblink(_dblink_name, 'select * from bc_identity_auth')
			as t(id integer, password character varying(32))
			where not exists (select 0 from bc_identity_auth r where r.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_identity_actor_relation(type_, master_id, follower_id, order_)
			select * from dblink(_dblink_name, 'select * from bc_identity_actor_relation')
			as t(type_ integer,
				master_id integer,
				follower_id integer,
				order_ character varying(100))
			where not exists (select 0 from bc_identity_actor_relation r where r.master_id = t.master_id and r.follower_id = t.follower_id and r.type_ = t.type_);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_wf_deploy(id, uid_, deployment_id, order_, status_, type_, category, code, 
			...
			select * from dblink(_dblink_name, 'select * from bc_wf_deploy where deployment_id = ''' || _deployment_id || '''')
			as t(id integer,
				uid_ character varying(36),
			...
			where not exists (select 0 from bc_wf_deploy r where r.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_template_type(id, status_, order_, code, name, is_path, is_pure_text, ext, 
			...
			select * from dblink(_dblink_name, 'select * from bc_template_type')
			as t(id integer,
				status_ integer,
				order_ character varying(255),
				code character varying(255),
				...
				modifier_id integer,
				modified_date timestamp without time zone)
			where not exists (select 0 from bc_template_type r where r.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_wf_deploy_resource(id, uid_, pid, code, subject, path, size_, source, type_id, formatted)
			select * from dblink(_dblink_name, 'select s.* from bc_wf_deploy_resource s 
			...
			as t(id integer,
				...
				type_id integer,
				formatted boolean)
			where not exists (select 0 from bc_wf_deploy_resource r where r.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_wf_deploy_resource_param(rid, pid)
			select * from dblink(_dblink_name, 'select p.* from bc_wf_deploy_resource_param p 
			...
			where not exists (select 0 from bc_wf_deploy_resource_param p where p.rid = t.rid and p.pid = t.pid);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_wf_excution_log(id, type_, listenter, eid, pid, tid, ecode, ename, formkey, desc_, 
			...
			select * from dblink(_dblink_name, 'select * from bc_wf_excution_log where pid = ''' || $1 || '''')
			as t(id integer,
				type_ character varying(255),
				...
				file_date timestamp without time zone)
			where not exists (select 0 from bc_wf_excution_log l where l.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_wf_attach(id, uid_, tid, pid, type_, common, subject, path_, ext, size_, 
			...
			select * from dblink(_dblink_name, 'select * from bc_wf_attach where pid = ''' || $1 || '''')
			as t(id integer,
				uid_ character varying(36),
				tid character varying(255),
				pid character varying(255),
				...
				modified_date timestamp without time zone,
				modifier_id integer)
			where not exists (select 0 from bc_wf_attach a where a.id = t.id);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
		...
		INSERT INTO bc_wf_attach_param(aid, pid)
			select * from dblink(_dblink_name, 'select p.* from bc_wf_attach_param p
				inner join bc_wf_attach a on a.id = p.aid where a.pid = ''' || $1 || '''')
			as t(aid integer, pid integer)
			where not exists (select 0 from bc_wf_attach_param p where p.aid = t.aid and p.pid = t.pid);
		GET DIAGNOSTICS _row_count = ROW_COUNT;
		--raise info 'row_count=%', _row_count;
		IF found THEN
