with t(task_id, task_name, task_status, start_time, end_time, duration, due_date, process_id, process_name
  ...
  as (
    ...
    from act_hi_taskinst t
      ...
      -- 条件
    where t.assignee_ != 'hrj' and t.end_time_ is not null and e.parent_id_ is null order by t.start_time_ desc
  )
...
where (
  exists(select 1 from act_hi_taskinst dc_t
    inner join act_re_procdef dc_r on dc_r.id_=dc_t.proc_def_id_
    inner join bc_wf_deploy dc_d on dc_d.deployment_id=dc_r.deployment_id_
    where dc_t.id_=t.task_id and dc_d.id in(6905)
  )
  or exists(select 1 from act_hi_taskinst pi_a where pi_a.id_=t.task_id and pi_a.proc_inst_id_ in('27650016')))
-- 分页
limit 25
