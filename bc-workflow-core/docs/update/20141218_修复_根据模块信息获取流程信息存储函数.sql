-- Function: wf_get_last_process_info(integer, character varying)

-- DROP FUNCTION wf_get_last_process_info(integer, character varying);

create or replace function wf_get_last_process_info(mid integer, mtype character varying)
  returns json as
$BODY$
/** 根据模块信息获取流程信息
 *  @param mid 模块Id
...
declare
  ...
  hi_inst json;
begin
  select row_to_json(act_hi_inst) into hi_inst
  from (
         select a.proc_inst_id_                as id, b.name_ as name -- 状态：1-流转中、2-已暂停、3-已结束
           ,
                (case a.end_time_ is not null when true
                  then 3
                 else f.suspension_state_ end) as status,
                ...
                to_json(
                  (-- 合并为json数组
                    ...
                    from (
                           ...
                           left join bc_identity_actor ia on ia.code = art.assignee_
                           -- 关联待办人表 ia.name
