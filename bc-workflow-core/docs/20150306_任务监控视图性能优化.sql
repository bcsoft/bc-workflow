...
from (
  ...
  , case when t.end_time_ is not null then 3 else
			(case when (pd.suspension_state_ = 2 or e.suspension_state_ = 2) then 2 else 1 end)
		end as task_status
  , t.start_time_ start_time, t.end_time_ end_time, t.duration_ duration, t.due_date_ as due_date
  , t.proc_inst_id_ process_id, pd.name_ process_name
  , a.name as assignee, e.suspension_state_ pstatus
  ...
  left join act_ru_execution e on e.proc_inst_id_ = t.proc_inst_id_ -- 暂停及流转中流程需要join此表
