...
public class Attach4TaskListener implements TaskListener {
  ...
  public void notify(DelegateTask delegateTask) {
    ...
    if (attachCode != null) {//编码不为空,根据编码从模板模块查找附件
      ...
      for (String code : attachCodes) {
        addAttach4Template(code, delegateTask);
      }
    }
    ...
    if (resourceCode != null) {//流程资源编码不为空,根据流程资源编码从流程资源获取信息生成附件
      ...
      for (String code : resourceCodes) {
        addAttach4Resource(code, delegateTask);
      }
    }

  }

  private void addAttach4Resource(String resourceCode, DelegateTask delegateTask) {
  ...
  private void addAttach4Template(String attachCode, DelegateTask delegateTask) {
    Template template;
