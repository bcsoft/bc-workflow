...
BEGIN
	...
	with instance(id, pid, duration, start_time, end_time, status, delete_reason, start_user, definition, deploy) as (
		...
		, (select row_to_json(t) from (
				...
				from act_re_procdef d where d.id_ = i.proc_def_id_
			) t) definition
		-- 流程部署
		, (select row_to_json(t) from (
				select p.id id, p.code as key, p.subject as name, p.version_ as version, p.deployment_id as deployment_id
				, p.desc_ as desc, to_char(p.deploy_date, 'YYYY-MM-DD HH24:MI:SS') deploy_time
				from bc_wf_deploy p
	...
	-- 全部流程变量的最新版ID
	, variable_id(id) as (
		select id_ id	from act_hi_detail h where h.proc_inst_id_ = $1 and type_ = 'VariableUpdate'
		-- 排除旧版本
		and not exists (
			select 0 from act_hi_detail h1
	...
	, variable(task_id, name, type, value) as (
		select h.task_id_ task_id, h.name_ as name, h.var_type_ as type
		, (case h.var_type_
				when 'boolean' then (case h.long_ when 1 then 'true' else 'false' end)
				when 'integer' then h.long_::text
				when 'long' then h.long_::text
	...
	, global_variable(name, type, value) as (
		select name, type, value from variable v where v.task_id is null
		order by type, name
	)
	-- 任务签领、委托日志（不含其它日志）
	, task_log(task_id, type, time, author, actor) as (
		select tid task_id, type_ as type, to_char(file_date, 'YYYY-MM-DD HH24:MI:SS') as time
		, (select row_to_json(u) from (select l.author_id id, l.author_code code, l.author_name as name) u) author
		, (select row_to_json(u) from (select l.assignee_id id, l.assignee_code code, l.assignee_name as name) u) actor
	...
	, task(id, pid, start_time, end_time, key, name, duration, due_date, priority, description, actor
		...
		, to_char(t.start_time_, 'YYYY-MM-DD HH24:MI:SS') start_time, to_char(t.end_time_, 'YYYY-MM-DD HH24:MI:SS') end_time
		, t.task_def_key_ as key, t.name_ as name
		, t.duration_ duration, to_char(t.due_date_, 'YYYY-MM-DD HH24:MI:SS') due_date, t.priority_ priority, t.description_ description
		-- 办理人
		, (
		...
		, t.assignee_ assignee, t.owner_ as owner
		-- 任务的本地变量
		, (select to_json(array_agg(row_to_json(v1))) from (
...
with wf(j) as (
	select wf__find_process_instance_detail('3766598')
)
-- 展开第一层的key
select * from json_each((select wf.j from wf))
union all select 'variables.len', to_json(json_array_length((select wf.j->'variables' from wf)))

-- 如果有待办
union all select 'todo_tasks[' || (row_number() over()) || ']' || (task.value->>'id'), task.value from json_array_elements((select wf.j->'todo_tasks' from wf)) task
union all select 'todo_tasks.len', to_json(json_array_length((select wf.j->'todo_tasks' from wf)))

-- 如果有经办
union all select 'done_tasks[' || (row_number() over()) || ']' || (task.value->>'id'), task.value from json_array_elements((select wf.j->'done_tasks' from wf)) task
union all select 'done_tasks.len', to_json(json_array_length((select wf.j->'done_tasks' from wf)))

-- 如果有任务签领、委托日志
union all select 'task_logs[' || (row_number() over()) || ']' || (task.value->>'task_id'), task.value from json_array_elements((select wf.j->'task_logs' from wf)) task
union all select 'task_logs.len', to_json(json_array_length((select wf.j->'task_logs' from wf)))
